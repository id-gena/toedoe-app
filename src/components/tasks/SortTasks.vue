<template>
    <Dropdown>
        <template v-slot:trigger="{ toggle, toggleClass }">
            <DropdownTrigger class="btn btn-light" :class="toggleClass" @click="toggle" type="button" hide-caret>
                <IconArrowDownUp />
            </DropdownTrigger>
        </template>

        <template v-slot:menu="{ toggle }">
            <DropdownItem v-for="item in menuItems" @click.prevent="toggle(), sortTask(item)" :key="item"
